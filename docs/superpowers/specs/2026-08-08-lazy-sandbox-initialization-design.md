...
Add a run-scoped `LazySandboxBackend` implementing `SandboxBackendProtocol`. It is created with:
...
Shell command strings are not rewritten because parsing arbitrary shell syntax is unsafe. The resolved provider session root remains the command working directory through the existing provider backends, and the lazy wrapper makes the same path available as `LAMBCHAT_WORKSPACE` for commands. Prompt text instructs the model to use relative paths or `$LAMBCHAT_WORKSPACE` for shell commands while using the public absolute path for file tools. Tools that synthesize shell commands from file paths, including `upload_url_to_sandbox`, must resolve the path through a narrow async backend path-resolution hook before building the command. This avoids embedding an unusable public path in provider-side shell commands.
...
6. Emit `sandbox:ready` with the real sandbox ID and the stable public work directory. Provider-specific actual paths remain internal implementation details.
...
On first sandbox use, all three providers emit one ordered `sandbox:starting` then `sandbox:ready` pair before the first operation result. Subsequent operations in the run reuse the resolved delegate. Prompts, environment variables, tools, skills/memory routes, artifacts, and terminal completion remain functionally equivalent to the eager path. The lifecycle events move to first use and expose a provider-neutral public work directory.
...
If initialization fails, the lazy backend records the exception, emits one `sandbox:error`, and re-raises a sanitized failure. Every waiter receives the same failure; retries within that run do not start another sandbox. No tool result may claim that an operation executed when initialization failed.

A caller cancelled while awaiting shared initialization stops waiting without cancelling the shared task or corrupting another waiter. A successfully created sandbox remains in the manager cache for later calls. If provider creation succeeds but binding, work-directory preparation, or environment synchronization fails, the manager's current provider-specific cleanup behavior remains authoritative.

Lifecycle writes are serialized by the one initialization task. Presenter emission failure is logged but does not replace a successful provider result; provider failure remains the causal exception. The wrapper never launches independent event-emission tasks, preventing duplicate or reordered `starting`, `ready`, and `error` events.
...
4. Representative async protocol tests cover execute and every path-bearing operation; protocol-shape/source coverage ensures no method bypasses `ensure_ready()` or path mapping.
5. Workspace tests cover public-to-actual and actual-to-public mapping, relative paths, segment boundaries, external absolute paths, shell cwd/environment behavior, and `upload_url_to_sandbox` resolution.
...
8. Cancellation tests prove one caller can stop waiting without corrupting another waiter or the eventual delegate.
...
10. Compatibility tests prove disabled sandbox mode remains unchanged and the three providers use one lazy Search Agent branch with no provider-specific eager fallback.
11. Existing Search Agent, provider backend, session manager, artifact delivery, subagent middleware, event, cancellation, and task executor tests remain green.
