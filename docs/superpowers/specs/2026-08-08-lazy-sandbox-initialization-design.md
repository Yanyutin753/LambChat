...
A caller cancelled while awaiting shared initialization stops waiting without cancelling the shared initialization task. The wrapper counts active waiters. If cancellation leaves no waiter, it marks initialization abandoned, suppresses later lifecycle events, transitions to a terminal cancelled state so another operation in that run cannot start a second provider request, and leaves the manager coroutine running to completion under `asyncio.shield`. Its done callback consumes success or failure so no unobserved task exception is produced.

The Search Agent owns the run-scoped wrapper and closes it from its cancellation/finalization path before emitting terminal completion. Closing marks the wrapper event-suppressed but does not cancel a manager coroutine that may already be inside non-cancellable thread-backed SDK work. The coroutine is allowed to finish binding/caching the sandbox or running the manager's normal failure cleanup, while the wrapper guarantees that it cannot emit late lifecycle events or resume the cancelled run. A successfully created sandbox remains in the manager cache for later runs. This avoids orphaning a remotely created sandbox whose SDK call completed after local cancellation.
...
8. Cancellation tests cover both cases: one caller stops waiting while another receives the eventual delegate; and the sole remaining caller is cancelled, causing event suppression before terminal completion while the shielded manager task finishes and its result/exception is consumed. Provider-adapter tests pause each of E2B, CubeSandbox, and Daytona inside its blocking create call, cancel the run, then prove a late success is bound/cached (or a late failure is cleaned up), with no orphan and no late ready/error event.
