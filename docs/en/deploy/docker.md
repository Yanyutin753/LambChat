...
File URLs generated within a request automatically follow the entry point the client used (`X-Forwarded-Host`/`Host` + `X-Forwarded-Proto`), so multi-domain deployments need no extra configuration. `APP_BASE_URL` is only a fallback when no request context exists; **it is recommended for URLs generated by background jobs** (scheduled-task attachments, message-interception artifacts, etc.):

```bash
APP_BASE_URL=https://lambchat.example.com
```

> **Upgrade note (connection settings move to env)**: connection-class settings — database (`MONGODB_*`/`POSTGRES_*`/`REDIS_*`/`CHECKPOINT_PG_*`), object storage (`S3_*`) and `ENABLE_POSTGRES_STORAGE` — are now **env-authoritative on startup**. Deployments that previously changed these via the settings panel must migrate the values to env/compose and restart; otherwise startup logs an `env-authoritative` warning and the panel values no longer apply. Behavioral settings in the panel (timeouts, toggles, ...) are unaffected.
