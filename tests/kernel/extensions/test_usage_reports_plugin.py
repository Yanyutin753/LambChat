...
def test_usage_reports_plugin_manifest_preserves_legacy_api_and_permissions() -> None:
    ...
    assert manifest.frontend.app_tabs[0].path == "/usage"
    assert manifest.frontend.app_panels[0].renderer == "usage_reports.UsagePanel"
    assert manifest.frontend.user_menu_items[0].path == "/usage"
...
def test_usage_reports_plugin_resources_enter_ledger() -> None:
    ...
    assert (PluginResourceType.APP_TAB, "usage_reports:usage-tab") in resource_keys
    assert (PluginResourceType.APP_PANEL, "usage_reports:usage-panel") in resource_keys
    assert (PluginResourceType.USER_MENU_ITEM, "usage_reports:usage-menu") in resource_keys
...
def test_usage_reports_uninstall_dry_run_keeps_usage_data_and_archives_indexes() -> None:
    ...
    assert actions_by_id["usage_reports:usage-tab"] is PluginDryRunAction.KEEP
    assert actions_by_id["usage_reports:usage-panel"] is PluginDryRunAction.KEEP
    assert actions_by_id["usage_reports:usage-menu"] is PluginDryRunAction.KEEP
