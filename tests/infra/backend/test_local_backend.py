...
# ============================================================================
# 统一确认门（服务端 HITL，spec §3.5）——exec/文件写/上传过门，读类绕过
# ============================================================================


@pytest.fixture(autouse=True)
def _gate_policy(monkeypatch):
    """确认策略打桩（autouse）：默认 none——既有用例不经门；门用例改 holder 值。

    单测不触真实 redis：不打桩时 _lookup_confirm_policy 会因 registry 故障
    归一 all，把所有既有 exec 用例拦在确认门上。
    """
    holder = {"policy": "none", "real_lookup": local_module._lookup_confirm_policy}

    async def fake_lookup(user_id):
        return holder["policy"]

    monkeypatch.setattr(local_module, "_lookup_confirm_policy", fake_lookup)
    return holder


@pytest.fixture
def _interrupt(monkeypatch):
    """langgraph interrupt 打桩：记录 payload，按 holder 决定行为。

    同时置 hitl_interrupt_supported=True（interrupt 模拟隐含图上下文可用，
    否则门会 fail-closed 直接拒绝而不触碰 interrupt）。

    - holder["resume"] 是 dict → 返回之（模拟用户已响应的重放）；
    - 否则 raise GraphInterrupt(payload)（模拟图任务内首次挂起）。
    """
    ...
    supported_token = hitl_interrupt_supported.set(True)
    holder: dict = {"resume": None, "payloads": []}

    def fake_interrupt(payload):
        holder["payloads"].append(payload)
        if holder["resume"] is not None:
            return holder["resume"]
        raise GraphInterrupt(payload)

    monkeypatch.setattr(langgraph.types, "interrupt", fake_interrupt)
    yield holder
    hitl_interrupt_supported.reset(supported_token)


async def test_exec_policy_none_dispatches_directly(monkeypatch, _gate_policy):
    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        return _ok_response(exit_code=0)

    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "none"
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
    resp = await backend.aexecute("rm -rf x")
    assert resp.exit_code == 0


async def test_exec_policy_all_interrupts_before_dispatch(monkeypatch, _gate_policy, _interrupt):
    dispatch_called = []

    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        dispatch_called.append(payload)
        return _ok_response()

    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "all"
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
    from langgraph.errors import GraphInterrupt
    ...
    assert _interrupt["payloads"][0]["kind"] == "ask_human"
    assert "ls -la" in _interrupt["payloads"][0]["message"]


async def test_exec_declined_returns_error_response(monkeypatch, _gate_policy, _interrupt):
    dispatch_called = []
    ...
    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "all"
    _interrupt["resume"] = {"approved": False, "values": {}}
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
...
async def test_exec_approved_dispatches(monkeypatch, _gate_policy, _interrupt):
    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        return _ok_response()

    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "all"
    _interrupt["resume"] = {"approved": True, "values": {}}
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
    resp = await backend.aexecute("rm x")
    assert resp.exit_code == 0


async def test_policy_lookup_failure_fails_closed(monkeypatch, _gate_policy, _interrupt):
    # 恢复真实 _lookup_confirm_policy，注入故障 registry：
    # 查询异常走其内部兜底归一 all → 门生效（fail-closed）
    class BoomRegistry:
        async def get_confirm_policy(self, user_id):
            raise RuntimeError("redis down")

    monkeypatch.setattr(local_module, "SandboxClientRegistry", BoomRegistry)
    monkeypatch.setattr(local_module, "_lookup_confirm_policy", _gate_policy["real_lookup"])
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
    from langgraph.errors import GraphInterrupt

    with pytest.raises(GraphInterrupt):  # 查询失败归一 all → 门生效
        await backend.aexecute("ls")


async def test_read_ops_bypass_gate(monkeypatch, _gate_policy, _interrupt):
    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        return _ok_response(stdout="")

    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "all"  # 读类绕过：all 也不触发 interrupt
    backend = local_module.WorkspaceAliasBackend(user_id="u1", session_id="s1")
    await backend.als(".")
...
async def test_write_gates_once_with_description(monkeypatch, _gate_policy, _interrupt):
    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        return _ok_response()

    monkeypatch.setattr(local_module, "dispatch_local_call", fake_dispatch)
    _gate_policy["policy"] = "commands"  # rm 哨兵命中
    _interrupt["resume"] = {"approved": True, "values": {}}
    backend = local_module.WorkspaceAliasBackend(user_id="u1", session_id="s1")
    result = await backend.awrite("a.txt", "hi")
    assert result.error is None
    assert len(_interrupt["payloads"]) == 1  # 写门一次，内部 upload 不再门
    assert "a.txt" in _interrupt["payloads"][0]["message"]


async def test_delete_declined_returns_error(monkeypatch, _gate_policy, _interrupt):
    dispatch_called = []
    ...
    _interrupt["resume"] = {"approved": False, "values": {}}
    backend = local_module.WorkspaceAliasBackend(user_id="u1", session_id="s1")
    result = await backend.adelete("/workspace/s1/a.txt")
    assert result.error is not None
    assert "declined_by_user" in result.error
    assert dispatch_called == []


async def test_upload_gates_once_per_batch_not_per_chunk(
    monkeypatch, _gate_policy, _interrupt
):
    dispatch_count = []

    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        dispatch_count.append(payload["command"])
        return _ok_response()
    ...
    _interrupt["resume"] = {"approved": True, "values": {}}
    backend = LocalSandboxBackend(user_id="u1", session_id="s1")
    big = b"x" * (49 * 1024)  # > 48KB → 两块命令
    responses = await backend.aupload_files([("a.bin", big)])
    assert len(_interrupt["payloads"]) == 1
    assert all(r.error is None for r in responses)
    assert len(dispatch_count) == 2


async def test_download_never_gates(monkeypatch, _gate_policy, _interrupt):
    async def fake_dispatch(user_id, op, payload, *, timeout=None):
        return _ok_response(stdout=base64.b64encode(b"data").decode())
