"""Local CDP smoke for approving a replayed paused workflow tool result."""
...
def click_workflow_tool_pill(client: CdpClient) -> None:
    wait_until(
        lambda: evaluate(
            client,
            """
(() => {
  const button = Array.from(document.querySelectorAll('button')).find((item) => {
    ...
    return text.includes('Workflow Run') || text.includes('workflow_run');
  });
...
(() => {
  ...
  if (!button) throw new Error('workflow tool pill not found');
  if (button.disabled) throw new Error('workflow tool pill disabled');
  button.click();
  return true;
})()
""",
...
def replay_messages_evidence(client: CdpClient) -> dict[str, Any]:
    return evaluate(
        client,
        """
(() => {
  const messages = window.__chatReplayWorkflowApprovalMessages || [];
  const assistant = messages.find((message) => message.role === 'assistant') || {};
  const parts = assistant.parts || [];
  return {
    messageCount: messages.length,
    assistantPartTypes: parts.map((part) => part.type),
    hasWorkflowToolPart: parts.some((part) => part.type === 'tool' && part.name === 'workflow_run'),
    hasPausedWorkflowResult: parts.some((part) =>
      part.type === 'tool' &&
      part.result &&
      part.result.plugin_id === 'workflow' &&
      part.result.status === 'paused' &&
      part.result.next_action?.resume?.tool === 'workflow_resume'
    ),
  };
})()
""",
...
def run_chat_replay_approval(base_url: str, chrome_path: Path, out_dir: Path) -> dict[str, Any]:
    stamp = time.strftime("%Y%m%d-%H%M%S")
    port = free_port()
    temp_profile = Path(tempfile.mkdtemp(prefix="lambchat-chat-replay-approval-cdp-"))
    ...
    try:
        ...
        navigate(client, base_url + CHAT_REPLAY_APPROVAL_SMOKE_PATH)
        wait_for_text(client, "chat session replay workflow approval tool smoke", timeout=60)
        replay_evidence = wait_until(
            lambda: evidence
            if (evidence := replay_messages_evidence(client)).get("hasPausedWorkflowResult")
            else None,
            timeout=45,
            interval=0.5,
        )
        click_workflow_tool_pill(client)
        ...
        screenshot_path = out_dir / f"workflow-chat-replay-approval-{stamp}.png"
        screenshot(client, screenshot_path)
        console_issues = render_console_issues(client.events)
        result = {
            "base_url": base_url,
            "path": CHAT_REPLAY_APPROVAL_SMOKE_PATH,
        ...
        result_path = out_dir / f"workflow-chat-replay-approval-{stamp}.json"
            ...
            json.dumps(
                {
                    ...
                    "has_replayed_paused_result": replay_evidence["hasPausedWorkflowResult"],
...
def evaluate_health(base_url: str) -> bool:
    ...
    response = requests.get(base_url + CHAT_REPLAY_APPROVAL_SMOKE_PATH, timeout=2)
    return response.ok
...
def main() -> int:
    ...
    try:
        ...
        run_chat_replay_approval(base_url, chrome_path, out_dir)
        return 0
    finally:
        if vite_proc:
            vite_proc.terminate()
