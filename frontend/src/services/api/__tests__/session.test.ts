...
test("builds session plugin options urls", () => {
  expect(buildSessionPluginOptionsUrl("session 1")).toBe("/api/sessions/session%201/plugin-options");
  expect(buildSessionPluginOptionUrl("session 1", "agent_team", "SELECTED_TEAM_ID")).toBe("/api/sessions/session%201/plugin-options/agent_team/SELECTED_TEAM_ID");
});
