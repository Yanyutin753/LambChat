    ...
    <div className={`space-y-3 ${className}`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-2">
          <div className="skeleton-line h-4 w-4 rounded-full flex-shrink-0" />
...
export function SkeletonCard({ className = "" }: SkeletonCardProps) {
  return (
    <div
      className={`p-3 rounded-lg bg-stone-50 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-700 ${className}`}
    >
      <div className="flex items-center gap-2 mb-2">
        <SkeletonBlock width="w-5" height="h-5" className="!rounded-full" />
