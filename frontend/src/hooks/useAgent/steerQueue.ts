...
interface SteerQueueOptions {
  ...
  deferSteer: (content: string, messageId: string, attachments?: MessageAttachment[]) => void;
}
 ...
 */
export function useSteerQueue({ sessionIdRef, deferSteer }: SteerQueueOptions) {
      ...
      try {
        ...
        if (status === 409) {
          setSteerMessages((prev) =>
            prev.map((s) =>
              s.id === item.id ? { ...s, queued: false, status: "deferred", deferred: true } : s,
            ),
          );
  ...
  const cancelSteer = useCallback(
    (content: string, messageId?: string) => {
      setSteerMessages((prev) =>
        prev.filter((s) => (messageId ? s.id !== messageId : s.content !== content)),
      );
      const currentSessionId = sessionIdRef.current;
      if (currentSessionId) {
        sessionApi.cancelSteer(currentSessionId, content, messageId).catch(() => {});
      }
    },
    [sessionIdRef],
  );

  const markSteerDelivered = useCallback((content: string, messageId?: string) => {
    setSteerMessages((prev) => {
  ...
  const clearSteer = useCallback(
    (content: string, messageId?: string) =>
      setSteerMessages((prev) =>
        prev.filter((item) => (messageId ? item.id !== messageId : item.content !== content)),
      ),
    [],
  );
