...
class TeamRouterDelegationGuardMiddleware(AgentMiddleware):
    ...
    async def awrap_tool_call(
        ...
        if not has_task_call and not _messages_contain(
            messages,
            _TEAM_ROUTER_BEFORE_DELEGATION_NUDGE,
        ):
            return self._tool_message(
                request,
                f"{_TEAM_ROUTER_BEFORE_DELEGATION_NUDGE} for substantive work they "
                "can handle. Call the `task` tool with the appropriate member, or "
                "retry this same tool call only if this is coordination, verification, "
                "packaging, or work no member is suitable for.",
            )

        if has_task_result and not _messages_contain(
            messages,
            _TEAM_ROUTER_AFTER_DELEGATION_NUDGE,
        ):
            return self._tool_message(
                request,
                f"{_TEAM_ROUTER_AFTER_DELEGATION_NUDGE}. Synthesize that result "
                "instead of redoing their work directly. Retry this tool call only "
                "for necessary fallback, verification, packaging, or clearly missing work.",
            )
    ...
    async def awrap_tool_call(
        ...
        return ToolMessage(
            content=(
                ...
                "Task type, Delivery mode, Reference policy, Tool policy, Max tool "
                "calls, Artifact intent, Target member, Context source, Allowed tools, "
                "Forbidden actions, Objective, Fixed inputs, and Output format. For "
                "complete text briefs use Tool policy: NO_TOOLS and Reference policy: "
                "USER_PROVIDED_ONLY."
            ),
            tool_call_id=request.tool_call.get("id", ""),
            name=tool_name,
...
class SubagentExecutionPolicyMiddleware(AgentMiddleware):
    """Soft-enforce the structured task policy for team subagents."""
    ...
    def _policy_decision(tool_name: str, text: str, fields: dict[str, str]) -> str | None:
        ...
        if tool_name in _TEXT_ONLY_ARTIFACT_TOOLS:
            return SubagentExecutionPolicyMiddleware._nudge(
                ...
                "content to be returned in text, not for files, folders, scripts, exports, "
                "or reveal links. Answer directly in text."
            )
        return None
