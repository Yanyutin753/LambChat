...
从运行目录读取 grading.json 文件，生成汇总统计数据：
- run_summary 包含每个指标的 mean, stddev, min, max
- with_skill 和 without_skill 配置之间的 delta

用法:
    python aggregate_benchmark.py <benchmark_dir> [--skill-name NAME]

示例:
    python aggregate_benchmark.py workspace/iteration-1 --skill-name my-skill
...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    返回按配置名（如 "with_skill"/"without_skill"）键控的字典，
    每个包含运行结果列表。
    """
    # 支持 eval 目录直接在 benchmark_dir 下或 runs/ 子目录下
    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                # 提取期望 - 需要 text, passed, evidence 字段
...
def aggregate_results(results: dict) -> dict:
    ...
    返回包含每个配置统计和 delta 的 run_summary。
    """
    run_summary = {}
...
def main():
    parser = argparse.ArgumentParser(description="聚合基准测试运行结果为汇总统计数据")
    parser.add_argument("benchmark_dir", type=Path, help="基准测试目录的路径")
    parser.add_argument("--skill-name", default="", help="正在基准测试的技能名称")
    parser.add_argument("--skill-path", default="", help="正在基准测试的技能路径")
    parser.add_argument(
        "--output",
        "-o",
        type=Path,
