...
class ErrorCode(Enum):
    ...
    REGISTRATION_SESSION_NOT_FOUND = ("registration_session_not_found", 404, "Registration session not found")

    OAUTH_PROVIDER_DISABLED = ("oauth_provider_disabled", 400, "OAuth provider '{{provider}}' is not enabled")
    ...
    PUSH_ENDPOINT_HTTPS_REQUIRED = ("push_endpoint_https_required", 400, "Push subscription endpoint must use HTTPS")
    PUSH_SUBSCRIPTION_KEYS_REQUIRED = ("push_subscription_keys_required", 400, "Push subscription keys are required")
    ...
    INVALID_PROFILE_FIELD_LIST = ("invalid_profile_field_list", 400, "Invalid {{field}}: must be a list of strings")
    PROFILE_FIELD_TOO_MANY = ("profile_field_too_many", 400, "Too many {{field}}: maximum {{max}} allowed")
    UNSUPPORTED_LANGUAGE = ("unsupported_language", 400, "Unsupported language: {{lang}}")
    INVALID_THEME = ("invalid_theme", 400, "Invalid theme: {{theme}}. Must be 'light', 'dark' or 'sepia'")
    ...
    INVALID_BEFORE_TRACE_STARTED_AT = ("invalid_before_trace_started_at", 400, "Invalid before_trace_started_at")
    ...
    DELETE_PROJECT_SESSIONS_FAILED = ("delete_project_sessions_failed", 500, "Failed to delete sessions in project")
    SESSIONS_NOT_IN_PROJECT = ("sessions_not_in_project", 400, "Some sessions do not belong to this project")
    ...
    SHARE_SESSION_ID_REQUIRED = ("share_session_id_required", 400, "Session share requires session_id")
    SHARE_EDIT_OWN_ONLY = ("share_edit_own_only", 403, "You can only edit shares you created")
    SHARE_PARTIAL_NEEDS_SESSION_IDS = ("share_partial_needs_session_ids", 400, "Partial project shares require session_ids")
    SHARE_PROJECT_NEEDS_PROJECT_ID = ("share_project_needs_project_id", 400, "Project share requires project_id")
    SHARE_RUN_IDS_LIMIT = ("share_run_ids_limit", 400, "run_ids cannot exceed {{max}} items")
    SHARE_SESSION_IDS_LIMIT = ("share_session_ids_limit", 400, "session_ids cannot exceed {{max}} items")
    ...
    PERSONA_PRESET_NOT_ALLOWED = ("persona_preset_not_allowed", 403, "Persona preset is not allowed")
    ...
    CHANNEL_AGENT_UNAVAILABLE = ("channel_agent_unavailable", 400, "Agent '{{agent_id}}' is not available")
    CHANNEL_AGENT_NOT_ALLOWED = ("channel_agent_not_allowed", 403, "Agent '{{agent_id}}' is not allowed for your role")
    CHANNEL_LARK_UNAVAILABLE = ("channel_lark_unavailable", 400, "lark-oapi register_app is unavailable")
    CHANNEL_LIST_LIMIT = ("channel_list_limit", 413, "Too many channel configurations to list at once (max {{max}})")
    UNKNOWN_CHANNEL_TYPE = ("unknown_channel_type", 404, "Unknown channel type: {{type}}")
    CHANNEL_TYPE_MISMATCH = ("channel_type_mismatch", 400, "Channel type mismatch: expected {{expected}}, got {{actual}}")
    ...
    COVER_THUMBNAIL_NOT_AVAILABLE = ("cover_thumbnail_not_available", 404, "Cover thumbnail not available")
    ...
    MARKETPLACE_FILES_INCOMPLETE = ("marketplace_files_incomplete", 500, "Marketplace skill files are incomplete")
    ...
    ONLY_ADMIN_OR_CREATOR_CAN_DELETE = ("only_admin_or_creator_can_delete", 403, "Only admin or creator can delete")
    INVALID_EXPIRES_RANGE = ("invalid_expires_range", 400, "expires must be between 60 and 86400 seconds")
    ...
    SKILL_BATCH_LIMIT = ("skill_batch_limit", 400, "Cannot process more than {{max}} skills at once")
    ...
    SKILL_FILE_NOT_FOUND = ("skill_file_not_found", 404, "File '{{path}}' not found in skill '{{skill}}'")
    ...
    SKILL_NOT_INSTALLED = ("skill_not_installed", 400, "Skill '{{name}}' not installed. Install it first")
    SKILL_MANUAL_NO_UPDATE = (
        "skill_manual_no_update",
        409,
        "Skill '{{name}}' is a manual skill and cannot be updated from marketplace",
    )
    MARKETPLACE_SKILL_NOT_FOUND = ("marketplace_skill_not_found", 404, "Marketplace skill '{{name}}' not found")
    MARKETPLACE_NAME_TAKEN = ("marketplace_name_taken", 409, "Marketplace skill name '{{name}}' is already taken")
    MARKETPLACE_FILE_COUNT_LIMIT = (
        "marketplace_file_count_limit",
        413,
        "Marketplace skill contains too many files (max {{max}})",
    )
    MARKETPLACE_FILE_TOO_LARGE = ("marketplace_file_too_large", 413, "Marketplace skill file is too large (max {{max}} characters)")
    MARKETPLACE_TOTAL_TOO_LARGE = (
        "marketplace_total_too_large",
        413,
        "Marketplace skill files are too large (max {{max}} total characters)",
    )
    GITHUB_INSTALL_LIMIT = ("github_install_limit", 400, "Cannot install more than {{max}} skills at once")
    ...
    MEMORY_IMPORT_LIMIT = ("memory_import_limit", 400, "Cannot import more than 1000 memories at once")
    MEMORY_MUST_BE_OBJECT = ("memory_must_be_object", 400, "Each memory must be an object")
    MEMORY_CONTENT_TOO_SHORT = ("memory_content_too_short", 400, "Memory content must be at least 5 characters")
    MEMORIES_MUST_BE_LIST = ("memories_must_be_list", 400, "memories must be a list")
    MEMORY_CONTENT_TOO_LARGE = ("memory_content_too_large", 400, "Memory content too large (max {{max}} characters)")
    MEMORY_IMPORT_TOO_LARGE = ("memory_import_too_large", 400, "Memory import content too large (max {{max}} total characters)")
    INVALID_MEMORY_TYPE = ("invalid_memory_type", 422, "Invalid memory_type. Must be one of: {{allowed}}")
    INVALID_MEMORY_SOURCE = ("invalid_memory_source", 422, "Invalid source. Must be one of: {{allowed}}")
    ...
    MCP_SYSTEM_SERVER_EXISTS = ("mcp_system_server_exists", 400, "System server '{{name}}' already exists")
    MCP_SYSTEM_SERVER_NOT_FOUND = ("mcp_system_server_not_found", 404, "System server '{{name}}' not found")
    ...
    MCP_IMPORT_LIMIT = ("mcp_import_limit", 413, "Import contains too many MCP servers (max {{max}})")
    ...
    MCP_SERVER_NOT_OWNED = ("mcp_server_not_owned", 404, "Server '{{name}}' not found or not owned by user")
    MCP_SERVER_ERROR = ("mcp_server_error", 400, "MCP server operation failed")
    MODEL_BATCH_LIMIT = ("model_batch_limit", 413, "Cannot process more than {{max}} models at once")
    INVALID_PROVIDER = ("invalid_provider", 400, "Invalid provider '{{provider}}'. Must be a non-empty string")
    ...
    HUMAN_RESUME_SUBMIT_FAILED = ("human_resume_submit_failed", 503, "Failed to submit resume task, please retry")
