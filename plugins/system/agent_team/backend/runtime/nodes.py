...
from plugins.system.agent_team.backend.runtime.asset_package_fallback import (
...
from plugins.system.agent_team.backend.runtime.delegation_events import (
    _build_forced_delegation_description,
    _build_full_asset_package_stage_description,
    _collect_full_asset_package_stages_from_event,
)
...
_FULL_ASSET_PACKAGE_TEAM_MARKERS = (
    "asset package",
    "first-frame",
    "first frame",
    "image-to-video",
...
_FULL_ASSET_PACKAGE_ASCII_INTENT_MARKERS = (
    "asset package",
    "first-frame",
    "first frame",
    "image-to-video",
...
def _looks_like_unreadable_user_input(user_input: Any) -> bool:
    text = str(user_input or "").strip()
    if not text:
        return False
    question_count = text.count("?")
    replacement_count = text.count("\ufffd")
    if question_count >= 6 and question_count / max(len(text), 1) >= 0.25:
        return True
...
def _team_text_for_routing(team: Any) -> str:
    fragments: list[str] = [
        str(getattr(team, "name", "") or ""),
        str(getattr(team, "description", "") or ""),
        str(getattr(team, "team_instructions", "") or ""),
    ]
    for member in getattr(team, "active_members", []) or []:
        fragments.extend(
            [
                str(getattr(member, "name", "") or ""),
                str(getattr(member, "role_name", "") or ""),
                str(getattr(member, "system_prompt", "") or ""),
            ]
        )
    return "\n".join(fragments).casefold()


def _is_full_asset_package_team(team: Any) -> bool:
    team_text = _team_text_for_routing(team)
    if not team_text:
        return False
    marker_hits = sum(1 for marker in _FULL_ASSET_PACKAGE_TEAM_MARKERS if marker in team_text)
    return marker_hits >= 2 or "素材包" in team_text


def _should_force_full_asset_package_for_team(*, team: Any, user_input: Any) -> bool:
    if not _is_full_asset_package_team(team):
        return False
    text = str(user_input or "").casefold()
    if _is_full_asset_package_request(text):
        return True
    if _looks_like_unreadable_user_input(text):
        return True
    if any(marker in text for marker in _FULL_ASSET_PACKAGE_ASCII_INTENT_MARKERS):
        return True
    return False
...
def _get_team_router_required_delegation_reason(*, team: Any, user_input: Any) -> str | None:
    if not team or not getattr(team, "active_members", None):
        return None
    if _should_force_full_asset_package_for_team(team=team, user_input=user_input):
        logger.info(
            "[TeamAgent] Team router resolved full asset package workflow: "
            "team_id=%s team_name=%s unreadable_input=%s",
            getattr(team, "id", None),
            getattr(team, "name", None),
            _looks_like_unreadable_user_input(user_input),
        )
