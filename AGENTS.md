...
| 云端沙箱暂停恢复（部署后，在可 kubectl 目标集群的机器上跑） | `python3 scripts/verify_sandbox_pause_heal.py --base-url <URL> --namespace <ns> --deployment <deploy> --user-id <uid>`（显式 pause→无感恢复；local 平台自动 SKIP；staging 可用 `--register` 一次性用户） |
